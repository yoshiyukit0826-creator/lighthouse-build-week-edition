...
`npm test` performs the verified production build and runs 23 tests: 3 pre-refactor characterization tests, 12 navigation-engine behavioral tests, 1 rendered HTML smoke test, and 7 source/CSS contract tests.
...
A later Codex core-engineering session rebuilt and hardened the Build Week navigation core after export, added characterization and behavioral tests, and wired the existing UI to the tested engine without redesigning it.
